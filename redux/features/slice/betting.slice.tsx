...
import type { LiveOutcome } from "../../services/data/betting.types";
// import { AppHelper } from "@/lib/helper";
...
const BettingSlice = createSlice({
  ...
  reducers: {
    ...
    updateStake: (
      ...
      // Recalculate winnings based on bet type
      if (state.coupon_data.bet_type === "Single") {
        ...
        const min_winnings = split_props.min_odds * state.coupon_data.min_stake;
        const max_winnings = split_props.max_odds * state.coupon_data.min_stake;
        ...
        state.coupon_data.max_win =
          state.coupon_data.gross_win - state.coupon_data.wth_tax;
      } else if (state.coupon_data.bet_type === "Combo") {
        const updated_coupon = updateComboWinningsFromTotal(
          state.coupon_data,
          global_vars,
          bonus_list
